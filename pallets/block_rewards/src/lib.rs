...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		/// The maximum allowed increase in argons per block in a single hour
		type MaxHourlyArgonIncreasePercent: Get<Percent>;

		/// The maximum allowed decrease in argons per block in a single hour
		type MaxHourlyArgonDecreasePercent: Get<Percent>;
	}
	...
	/// The argons per block trailing history
	#[pallet::storage]
	pub(super) type ArgonsPerBlockHistory<T: Config> =
		StorageValue<_, BoundedVec<T::Balance, ConstU32<60>>, ValueQuery>;
		...
		T::Balance: Into<u128>,
	{
		...
		pub(crate) fn adjust_block_argons(argon_minimum: T::Balance) {
			...
			if block_argons.is_zero() {
				ArgonsPerBlock::<T>::put(argon_minimum);
				block_argons = argon_minimum;
			}
			let mut block_history = ArgonsPerBlockHistory::<T>::get();
			if !argon_cpi.is_zero() {
				block_argons = argon_cpi.saturating_mul_acc_int(block_argons);

				let period_anchor_value = block_history.first().unwrap_or(&block_argons);
				let hour_decrease_clamp =
					T::MaxHourlyArgonDecreasePercent::get().mul_ceil(*period_anchor_value);
				let hour_increase_clamp =
					T::MaxHourlyArgonIncreasePercent::get().mul_ceil(*period_anchor_value);
				// don't allow changes to exceed the max change per hour
				block_argons = block_argons
					.clamp(
						period_anchor_value.saturating_sub(hour_decrease_clamp),
						period_anchor_value.saturating_add(hour_increase_clamp),
					)
					.max(argon_minimum);

				ArgonsPerBlock::<T>::set(block_argons);
			}

			if block_history.is_full() {
				block_history.remove(0);
			}
			let _ = block_history.try_push(block_argons);
			ArgonsPerBlockHistory::<T>::set(block_history);
		}
