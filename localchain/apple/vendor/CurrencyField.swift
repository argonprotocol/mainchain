...
public struct CurrencyTextField: UIViewRepresentable {
  ...
  public func makeUIView(context: UIViewRepresentableContext<CurrencyTextField>) -> UITextField {
    ...
    textField.addTarget(
      context.coordinator,
      action: #selector(context.coordinator.textFieldEditingDidBegin(_:)),
      for: .editingDidBegin
    )
    textField.addTarget(
      context.coordinator,
      action: #selector(context.coordinator.textFieldEditingDidEnd(_:)),
      for: .editingDidEnd
    )
    ...
    if let v = value {
      textField.text = v.currencyFormat(
        ...
        forceShowDecimalPlaces: alwaysShowFractions,
  ...
  public func updateUIView(_ textField: UITextField, context: UIViewRepresentableContext<CurrencyTextField>) {
    ...
    if value != context.coordinator.internalValue {
      if value == nil {
        ...
        textField.text = value!.currencyFormat(
          decimalPlaces: numberOfDecimalPlaces,
          forceShowDecimalPlaces: alwaysShowFractions,
          currencySymbol: currencySymbol
        )
    ...
    if isResponder?.wrappedValue == true && !textField.isFirstResponder && !context.coordinator
  ...
  public class Coordinator: NSObject, UITextFieldDelegate {
    ...
    public func textField(
      ...
      if textField.text == display && !string.isEmpty {
        return false
      }
      ...
      if string.isEmpty && originalText == display {
...
private extension Double {
  ...
  func currencyFormat(
    ...
    forceShowDecimalPlaces: Bool = false,
    currencySymbol: String? = nil
  ) -> String? {
    let formatter = Formatter.currency
    var integer = 0.0
