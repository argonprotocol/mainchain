	...
	C: AuxStore,
{
	...
	pub(crate) async fn enqueue(
		...
		if let Err(err) = self.persist_snapshot(&pending_imports) {
			pending_imports.retain(|entry| entry.hash != hash);
			return Err(err);
		}
		...
		C: BlockBackend<B>,
	{
		...
		sort_pending_imports(&mut pending_imports);
		if let Err(err) = self.persist_snapshot(&pending_imports) {
			warn!(error = ?err, "Failed to persist pruned pending import queue");
		}
			...
			*self.replay_scan_cursor.lock().expect("replay scan cursor lock poisoned") =
			...
			if self.pending_import_ready_for_replay(notary_client, replay_context).await {
					...
					pending_imports.iter().position(|entry| entry.hash == replay_context.hash)
				{
					...
					if let Err(err) = self.persist_snapshot(&pending_imports) {
						pending_imports.insert(index, pending_import);
						return Err(err);
					}
	...
	async fn requeue_pending_import(
		...
		let pending_hash = pending_import.hash;
		let mut pending_imports = self.pending_imports.lock().await;
		...
		sort_pending_imports(&mut pending_imports);
		if let Err(err) = self.persist_snapshot(&pending_imports) {
			pending_imports.retain(|entry| entry.hash != pending_hash);
			return Err(err);
		}
		Ok(())
	}

	fn persist_snapshot(&self, pending_imports: &[PendingBlockImport<B>]) -> Result<(), Error> {
		persist_pending_imports_to_aux::<B, C>(&self.client, pending_imports).map_err(|err| {
			Error::PendingImportPersistence(format!(
				"failed to persist {} deferred full imports: {err}",
				pending_imports.len()
			))
		})
	}
		...
		async move {
			...
			loop {
				...
				if let Err(err) = replay_pending_imports().await {
					panic!("pending import replay task failed: {err}");
				}
			}
		}
		.boxed(),
...
mod tests {
	...
	async fn test_bounded_replay_scan_advances_cursor_after_blocked_batch() {
		...
		assert!(
			queue.dequeue_ready_for_replay(&notary_client).await.unwrap().is_none(),
			"the first bounded pass should only see blocked entries",
		);

		let second_batch = queue.dequeue_ready_for_replay(&notary_client).await.unwrap();
		assert_eq!(
