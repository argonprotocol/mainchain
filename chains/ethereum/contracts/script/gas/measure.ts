...
import {
  ...
  type MintingGatewayV2MintingAuthorization,
  type MintingGatewayV2TransferOutOfArgonProof,
...
type GatewayContract = {
  ...
  finalizeTransferOutOfArgon(
    ...
    proof: MintingGatewayV2TransferOutOfArgonProof,
  ): Promise<TransactionLike>;
  cancelTransferOutOfArgon(request: MintingGatewayV2TransferOutOfArgonRequest): Promise<TransactionLike>;
  applyGatewayUpdates: ((
    currentCouncil: MintingGatewayV2CouncilSnapshot,
    updates: readonly MintingGatewayV2GatewayUpdate[],
...
type DeployStack = Awaited<ReturnType<typeof deployGatewayV2Stack>>;
...
async function main() {
  try {
    ...
    console.log('- chained queue hashes mainly help when multiple council-approved items share one council segment.');
    console.log('- weiAt10Gwei / weiAt20Gwei and ethAt10Gwei / ethAt20Gwei are gas * gasPrice only.');
  } finally {
...
async function measureSetupAndCouncilPaths() {
  ...
  const fourSetup = await deployGatewayV2Stack(fourCouncil);
  const hundredSetup = await deployGatewayV2Stack(hundredCouncil);
  ...
  const hundredActivationBatchGas =
    await measureMintingAuthorityActivationBatchGas(hundredBatchSetup, 1n, 3);
  const fourRotationGas = await measureCouncilRotationGas(fourSetup, 2n, createCouncil(4));
  const hundredRotationGas =
    await measureCouncilRotationGas(hundredSetup, 2n, createCouncil(100));
...
async function measureUserPaths() {
  ...
  const finalizeThreeAuthorityGas = await getGasUsed(
    multiAuthorityFixture.gateway.connect(multiAuthorityFixture.outsider).finalizeTransferOutOfArgon(
      multiRequest,
      { authorizations: multiAuthorizations },
    ),
  );
...
async function deployFixture(council: Council) {
  const stack = await deployGatewayV2Stack(council);
...
async function deployGatewayV2Stack(council: Council) {
  const [, adminSafe, guardian, holder, recipient, outsider] =
    (await ethers.getSigners()) as AccountSigner[];

  const gatewayFactory =
    (await ethers.getContractFactory('MintingGatewayV2')) as unknown as ContractFactory<ContractWithDeployment>;
  const gatewayBootstrapImplementation =
    await gatewayFactory.deploy(ethers.ZeroAddress, ethers.ZeroAddress);
  ...
  const proxyAdmin =
    (await ethers.getContractAt('ProxyAdmin', proxyAdminAddress)) as unknown as ProxyAdminContract;

  const argonFactory =
    (await ethers.getContractFactory('ArgonToken')) as unknown as ContractFactory<TokenContract>;
  const argonotFactory =
    (await ethers.getContractFactory('ArgonotToken')) as unknown as ContractFactory<TokenContract>;
...
async function measureMintingAuthorityActivationBatchGas(
  ...
  const updates: MintingGatewayV2GatewayUpdate[] = [];
...
): Promise<MintingGatewayV2MintingAuthorization> {
  const authorizationHash = hashMintingGatewayV2MintingAuthorization(
...
function createCouncil(memberCount: number): Council {
  const wallets = Array.from({ length: memberCount }, () => Wallet.createRandom() as unknown as SignerLike).sort(
    (left, right) => left.address.toLowerCase().localeCompare(right.address.toLowerCase()),
  );
  const weights =
    memberCount === 4
      ? [...COUNCIL_WEIGHTS]
      : Array.from({ length: memberCount }, () => 1n);
...
async function getGatewayHashContext(
  gateway: { getAddress(): Promise<Address> },
...
async function getGasUsed(
  action: Promise<TransactionLike>,
) {
