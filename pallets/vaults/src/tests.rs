...
fn it_can_set_securitization_ratio_for_a_vault() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(vault.available_for_lock(), 50_000);
	});
}
...
fn it_can_modify_a_vault_funds() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(VaultsById::<Test>::get(1).unwrap().available_for_lock(), 1000);
		System::assert_last_event(
			Event::VaultModified {
				vault_id: 1,
				securitization: 1000,
...
fn it_can_close_a_vault() {
	new_test_ext().execute_with(|| {
		...
		let amount = 40_000;
		let fee = Vaults::lock(1, &2, &securitization(amount), 500, None).expect("bonding failed");
		assert_eq!(fee, 401);
		let vault = VaultsById::<Test>::get(1).unwrap();
		...
		assert_err!(Vaults::lock(1, &2, &securitization(1000), 500, None), VaultError::VaultClosed);
	});
}
...
fn it_can_lock_funds() {
	new_test_ext().execute_with(|| {
		...
		set_argons(2, 6_000);
		let fee = Vaults::lock(1, &2, &securitization(500_000), 500, None).expect("bonding failed");
...
fn it_doesnt_charge_lock_fees_to_operator() {
	new_test_ext().execute_with(|| {
		...
		let fee = Vaults::lock(1, &1, &securitization(500_000), 500, None).expect("bonding failed");
...
fn it_accounts_for_pending_bitcoins() {
	new_test_ext().execute_with(|| {
		...
		let _ = Vaults::lock(1, &2, &securitization(100_000), 500, None).expect("bonding failed");
...
fn it_can_burn_funds() {
	new_test_ext().execute_with(|| {
		...
		set_argons(2, 2_000);
		let fee = Vaults::lock(1, &2, &securitization(100_000), 500, None).expect("bonding failed");
		...
		assert_ok!(Vaults::burn(1, &securitization(100_000), 100_000, &LockExtension::new(2440)));
...
fn it_can_cleanup_at_bitcoin_heights() {
	new_test_ext().execute_with(|| {
		...
		CurrentTick::set(1);
		assert_ok!(Vaults::lock(1, &2, &securitization(amount), 500, None));
		assert_eq!(
		...
		// assume the bitcoin locks pallet will clean this up
		VaultsById::<Test>::mutate(1, |vault| {
			if let Some(vault) = vault {
				vault.securitization_pending_activation = 0;
			}
...
fn it_can_reuse_locked_argons() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Vaults::lock(1, &3, &securitization(2_500_000), 2500, None));
		assert_eq!(
			VaultsById::<Test>::get(1).unwrap().available_for_lock(),
