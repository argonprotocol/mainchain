...
macro_rules! inject_runtime_vars {
	() => {
		...
		type Migrations = (
			...
			pallet_crosschain_transfer::hyperbridge_migration::InitializeCrosschainTransfer<
				Runtime,
			>,
			...
			pallet_ethereum_verifier::migrations::MigrateExecutionHeaderAnchorsV0ToV1Migration<
				Runtime,
			>,
		);
