...
fn it_wont_accept_bids_until_bidding_starts() {
	...
	new_test_ext().execute_with(|| {
		...
		System::set_block_number(12 + TicksBetweenSlots::get() - 1);
...
fn it_should_allow_each_miner_to_get_full_rewards() {
	...
	new_test_ext().execute_with(|| {
		...
		for block_number in 3..12 {
			...
			System::initialize(&block_number, &System::parent_hash(), &Default::default());
			MiningSlots::on_initialize(block_number);
			MiningSlots::on_finalize(block_number);
		}
		assert_eq!(NextFrameId::<Test>::get(), 2);
	...
	new_test_ext().execute_with(|| {
		...
		let mut winners_by_id = HashMap::<u64, u16>::new();
			...
			for i in 1..=10_000u64 {
				current_tick += 1;
				System::set_block_number(start_block_number + i - 1);
				System::initialize(
...
fn it_adjusts_mining_seats() {
	...
	new_test_ext().execute_with(|| {
		...
		for frame in 1..=11 {
			...
			FrameRewardTicksRemaining::<Test>::set(1);
			System::set_block_number(20 + frame * 10 - 1);
...
fn it_doesnt_accept_bids_until_first_slot() {
	...
	new_test_ext().execute_with(|| {
		...
		System::set_block_number(next_divisible_period - 1);
...
#[test]
fn grandpa_authority_derivation_limits_weighted_authorities_to_vote_budget() {
	let original_total_vote_weight = GrandpaTotalVoteWeight::get();
	GrandpaTotalVoteWeight::set(3);
	...
	assert_eq!(authorities, vec![(1u64.into(), 1), (2u64.into(), 1), (3u64.into(), 1)]);

	GrandpaTotalVoteWeight::set(original_total_vote_weight);
}
