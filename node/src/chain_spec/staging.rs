...
pub fn staging_config() -> Result<ChainSpec, String> {
	...
	const HASHES_PER_SECOND: u64 = 5000;
	...
	Ok(ChainSpec::builder(WASM_BINARY.ok_or_else(|| "Wasm not available".to_string())?, None)
		...
		.with_id("argon-staging")
		.with_protocol_id("stargon")
		...
		.with_genesis_config_patch(testnet_genesis(GenesisSettings {
			...
			initial_notaries: vec![GenesisNotary {
				...
				hosts: vec!["wss://notary1.staging.argonprotocol.org".to_string().into()],
			...
			mining_config: MiningSlotConfig {
				...
				slot_bidding_start_after_ticks: 14_400,
			},
